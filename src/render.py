...
class RenderStats():
    ...
    STYLES = {
        'titles': ImageFont.truetype("fonts/MyriadPro-Bold.otf", 10),
        'main': ImageFont.truetype("fonts/MyriadPro-Bold.otf", 20),
        'rank': ImageFont.truetype("fonts/MyriadPro-Bold.otf", 15)
    }
    METRICS = [
        'DAMAGE',
        'KILLS',
    ...
    def background_load(self):
        return Image.open('img/{}.jpg'.format(self.match.map_name))
    ...
    def calc_player_shots(self, index, mate, attacks, damages):
        m_attacks = [attack for attack in attacks
            if attack.attacker.account_id == mate.player_id
            and attack.weapon.category == 'Weapon'
            and attack.weapon.sub_category in ['Main', 'Handgun']]

        m_damages = [damage for damage in damages
        ...
        m_used_weapon = {x.weapon.item_id:x.fire_weapon_stack_count
            for x in m_attacks}
        shots = sum(m_used_weapon.values())
        hits = len([x for x in m_damages if x.damage_reason in
            ["ArmShot","HeadShot","LegShot","PelvisShot","TorsoShot"]])
        headshots = len([x for x in m_damages
    ...
    def calc_player_lifetime(self, index, mate, positions):
        m_positions = [pos for pos in positions
    ...
    def calc_player_dbnos(self, index, mate, dbnos):
        ...
        m_dbnos = [dbno for dbno in dbnos
    ...
    def calc_player_revived(self, index, mate, revives):
        ...
        m_revives = [revive for revive in revives
    ...
    def calc_player_kill(self, index, mate, kills):
        ...
        m_kills = [kill for kill in kills
    ...
    def calc_vehicle_rides(self, index, mate, rides):
        ...
        m_rides = [ride for ride in rides
            if ride.character.account_id == mate.player_id
            and ride.vehicle.vehicle_type in ['FloatingVehicle', 'WheeledVehicle']]
        for ride in m_rides:
            if ride.character.account_id != mate.player_id:
                continue
    ...
    def calc_vehicle_leaves(self, index, mate, leaves):
        ...
        m_leaves = [leave for leave in leaves
            if leave.character.account_id == mate.player_id
            and leave.vehicle.vehicle_type in ['FloatingVehicle', 'WheeledVehicle']]
    ...
    def draw_timeline_legend(self):
        ...
        _padding += 75
        self.draw.rectangle([(_padding, margin + 1),(_padding + 6, margin + 7)],
    ...
    def draw_timeline(self):
        ...
        for index, mate in enumerate(self.teammates):
            ...
            for index, ride in enumerate(mate.vehicle_rides):
                ...
                try:
                    end = time_map(duration, mate.vehicle_leaves[index].elapsed_time, max_width)
                except IndexError:
                    end = start 
            ...
            for index, dbno in enumerate(mate.dbnos):
                ...
                try:
                    end = time_map(duration, mate.revived[index].elapsed_time, max_width)
                except IndexError:
                    end = width
                
                self.draw.rectangle([(padding + start, margin), (end, margin + height)],
                    fill=self.TL_COLORS['dbno'])
